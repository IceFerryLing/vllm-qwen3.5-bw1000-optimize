...
@pytest.mark.parametrize("device", CUDA_DEVICES)
@torch.inference_mode()
def test_cp_gather_kv_cache_matches_separate(
    use_seq_starts: bool,
    dtype: torch.dtype,
    device: str,
) -> None:
    ...
    num_heads = 4
    head_size = 256
    seq_lens = torch.tensor([0, 17, 31, 48], dtype=torch.int32, device=device)
    cu_seq_lens = torch.zeros(batch_size + 1, dtype=torch.int32, device=device)
    cu_seq_lens[1:] = seq_lens.cumsum(dim=0)
    seq_starts = (
        torch.tensor([3, 7, 16, 5], dtype=torch.int32, device=device)
        if use_seq_starts
        else None
    )
    ...
    output_shape = (total_tokens, num_heads, head_size)
    ref_key = torch.empty(output_shape, dtype=dtype, device=device)
    ref_value = torch.empty_like(ref_key)
    dst_key = torch.empty_like(ref_key)
    dst_value = torch.empty_like(ref_key)
    ...
    torch.testing.assert_close(dst_key, ref_key, atol=0, rtol=0)
    torch.testing.assert_close(dst_value, ref_value, atol=0, rtol=0)
