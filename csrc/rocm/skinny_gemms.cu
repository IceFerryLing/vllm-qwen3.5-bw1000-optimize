...
torch::Tensor rocblas_bf16_mlp_gate_up_4096(const at::Tensor& weight,
  ...
  constexpr int64_t M = 34816;
  constexpr int64_t K = 5120;
  constexpr int32_t SOLUTION_INDEX = 20981;

  TORCH_CHECK(input.dim() == 2 && input.size(0) == N && input.size(1) == K,
              "input must have shape [4096, 5120]");
  TORCH_CHECK(weight.dim() == 2 && weight.size(0) == M && weight.size(1) == K,
              "weight must have shape [34816, 5120]");
  TORCH_CHECK(input.scalar_type() == torch::kBFloat16 &&
                  weight.scalar_type() == torch::kBFloat16,
              "rocblas_bf16_mlp_gate_up_4096 requires BF16 tensors");
  ...
  status = rocblas_gemm_ex(
      ...
      rocblas_datatype_bf16_r, M, output.data_ptr(), rocblas_datatype_bf16_r, M,
      rocblas_datatype_f32_r, rocblas_gemm_algo_solution_index, SOLUTION_INDEX,
      0);
  TORCH_CHECK(status == rocblas_status_success,
              "rocblas_gemm_ex solution 20981 failed with status ", status);
  return output;
}
